...
export default function Products() {
  ...
  return (
    <main>
      <Helmet>
        <title>Products — Premium Indonesian Spices for Export</title>
        <meta name="description" content="Explore export-grade Indonesian cinnamon, clove, nutmeg, black pepper, and white pepper from PT Sukses Ekspor Mandiri." />
        <meta property="og:title" content="Products — Premium Indonesian Spices for Export" />
      </Helmet>
            ...
            {FILTERS.map((f) => (
              <button
                ...
                className={`rounded-full px-5 py-2.5 text-sm font-semibold transition-colors ${
                  filter === f ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground hover:bg-gold-soft"
                }`}
              >
                {f}
              </button>
